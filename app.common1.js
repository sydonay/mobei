...
['$log',
	function($log){
		...
		function getHostPath(suffix){
			...
			//静态本地调试时打开注释
			//localhostPath='http://192.168.0.19:8080/';
...
['$timeout','$compile','$log',
	function($timeout,$compile,$log){
		...
		this.pop = function(popParams){
			var currPayParams = {title:'提示',info:'提示',okClickHandler:function(){},onCancelClickHandler:function(){}, buttonText1: '确定', buttonText2: "取消"};
			...
			if(!globalToastBox){
				...
				if(currPayParams.okClickHandler != null){
					title += '       <span style="display:inline-block;width:50%;float:left;padding:12px 0;text-align:center;color:#12c3b1;border-right:1px solid #bcbcbc;" ng-click="onOKClick()">'+currPayParams.buttonText1+'</span> 		';
				}
				if(currPayParams.onCancelClickHandler != null){
					title += '       <span style="display:inline-block;width:50%;float:left;padding:12px 0;text-align:center;color:#12c3b1;" ng-click="onCancelClick()">'+currPayParams.buttonText2+'</span>                         ';
				}
				title += '     </div>';
				title += '   </div>';
				title += ' </div>';
...
['$timeout','$compile','$log',
	function($timeout,$compile,$log){
		...
		this.pop = function(popParams){
			var currPayParams = {info:'提示',okClickHandler:function(){},onCancelClickHandler:function(){}, buttonText1: '知道了'};
			angular.extend(currPayParams, popParams);                                                                                                                                              
			...
			if(!globalToastBox){
				...
				if(currPayParams.okClickHandler != null){
					title += '       <span style="display:inline-block;width:100%;font-size:18px;padding-top:10px;text-align:center;color:#12c3b1;" ng-click="onOKClick()">'+currPayParams.buttonText1+'</span> 		';
				}
				title += '     </div>                                                                                                                                                       ';
				title += '   </div>                                                                                                                                                         ';
				title += ' </div>                                                                                                                                                           ';
	...
	function ($compile, $timeout) {
		...
		this.open = function (leftJSON, rightJSON, selectParams) {
			...
			if (!globalToastBox) {
				...
				box += '      <div ng-click="done()" style="text-align:right;padding:20px 14px 0 14px;">完成</div>';
				box += '      <div style="height:'+(currParams.height)*5+'px;overflow:hidden;padding:0 14px;position:relative;margin:40px 0;">';
				...
				box += '                      <li style="list-style:none;height:'+currParams.height+'px;line-height:'+currParams.height+'px;" ng-repeat="l in  leftData" ng-bind="l.month"></li>';
				box += '                  </ul>';
				box += '              </div>';
				box += '              <div style="width:50%;float:left;">';
				box += '                  <ul id="rightSelect" style="list-style:none;position:relative;top:0;left:0;text-align:center;">';
				box += '                      <li style="list-style:none;height:'+currParams.height+'px;line-height:'+currParams.height+'px;" ng-repeat="r in rightData" ng-bind="r.year"></li>';
				box += '                  </ul>';
				box += '              </div>';
...
.filter('fontFamily',function(){
	return function (input){
		var patt = new RegExp(/([A-Za-z0-9]+)/g);
		var result; //匹配结果
		var keys = []; //关键字
		//var index = []; //关键字开始索引
		//var lastIndex = []; //关键字结束索引
		var html = []; //html文本
		while ((result = patt.exec(input)) != null)  {
			keys.push(result[0]);
			//index.push(result.index);
			//lastIndex.push(patt.lastIndex);
		}
		for(var i=0;i<keys.length;i++){
			html[i] = '<span class="fix_family">'+keys[i]+'</span>';
			input = input.replace(keys[i],html[i]);
		}
		return input;
	} 
})
